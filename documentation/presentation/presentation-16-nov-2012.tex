...
\newcommand*\subkop{Solving the Bolognese process scheduling problem}
\newcommand*\naam{Alon Dolev, Joey Ezechi\"{e}ls, Volker Lanting}
...
\section{The Bolognese process scheduling problem}
	\frame{\frametitle{Outline}\tableofcontents[currentsection,currentsubsection]}
\newpic{images/BPLogo.jpg}
\frame{\frametitle{The Bolognese process scheduling problem}
...
\section{Our approach}
	...
	\frame{\frametitle{User interface with Flapjax}
		What is Flapjax?
		\begin{itemize}
			\item functional \emph{reactive} programming
			\item uses concept of dataflows
			\item dataflow can be extracted
			\item dataflows can be merged
			\item dataflows can be transformed
			\item we react on changes in dataflows
		\end{itemize}
		What if we want to change \\the \emph{structure} of the UI?
	}
	\nopic

	\subsection{Data transformation with Underscore.js}
	\frame{\frametitle{Outline}\tableofcontents[currentsection,currentsubsection]}
	\newpic{images/lego.jpg}
	\frame{\frametitle{Data transformation with Underscore.js}
		\begin{itemize}
			\item obtain data from the DOM
			\item series of transformations
...
\section{What we learned}
	...
	\frame{\frametitle{What we learned}
		\begin{tikzpicture}
			...
			\node at (0, 2.6)
			{\includegraphics[width=3cm]{images/underscore.png}};
			\node at (0,0)
...
\section{Questions}
	...
	\frame{\frametitle{Picture credit}
		spaghetti bolognese : http://www.telegraph.co.uk/foodanddrink \\
		kid with lego tower : http://www.howtorunakidsparty.com \\
		flapjacks : http://www.atomicrobotics.com/2012/02/flapjacks \\
		oscar from sesamstraat : http://www.wiezoekje.net/oscar-sesamstraat \\
		oscar logo: https://bitbucket.org/oscarlib \\
		underscore logo : http://underscorejs.org \\
		flapjax logo : http://flapjax-lang.org \\
		scala logo : http://scala-lang.org \\
		bologna logo : http://www.ond.vlaanderen.be \\
		kid in box : http://moveoneinc.com \\
		binary digits : http://aboutfacts.net/ComputRobot62.htm
	}
