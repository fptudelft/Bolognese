 ...
 * Currently it only contains a function to create a Constraint Model for the Bolognese problem.
 */
object ConstraintModel {
	 ...
	 * @param categories: the Categories of the problem instance, in which Modules can be booked
	...
	def fromBolognese(modules:List[Module], categories:List[Category], totalEcts:Int) : CPModel = {
		...
		// create a Map of Categories to the sum of the ECTS of the booked Modules for this Category
		...
		// create constraint for each Module, that it can only be booked in one Category at a time
		val moduleBookingConstraints = for ((m,cs)<-mc) yield {
			CPMIntLtEq(CPMIntSum(cs.map(c=>CPMIntRef(m.id+":"+c.id))), CPMIntCons(1))
		}
		// create a constraint that the total amount of creditable booked ECTS is at least totalEcts
