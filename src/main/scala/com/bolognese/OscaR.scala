...
/*
 * The OscaR monad is a monad which does some OscaR computations, and then returns the result.
 * It can be viewed as a box with something, and when you take the something out of the box, side-effects happen.
 * 
 * The only OscaR a monad that initially makes sense is the OscaR Collection[CPMFixedVar] monad,
 * which can be constructed with a CPModel.
 * It runs some OscaR computations based on the CPModel and returns the collection of fixed variable values,
 * as solved by OscaR.
 * To encapsulate the side-effects of OscaR, these computations will not happen until you extract the
 * value using the apply function.
 * Use map and flatMap to map the CPFixedVars to a suitable solution and then only at the very end 
 * run apply to get the value back out.
 * 
