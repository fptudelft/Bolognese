$(document).ready(function(){
  ...
  var close = constantB(-1); // The value was chosen because it is never used: Points can't be negative
  ...
  var categoriesAllE = categoryNameE.mergeE(categoryMinE, categoryMaxE, categoriesDelE).delayE(5);
  ...
  var categoriesCollectedE = categoriesAllE.collectE([], function(item, rest) {
      /*
        ...
        return rest;
      }).mapE(function(a) { return TABLE(TR(TH("Name"), TH("Minimum Points"), TH("Maximum Points")), a); });
  ...
  /*
    ...
    */
    ...
    var modulesFormValidB = andB(moduleNameValidB, modulePointsValidB,
 ...
 var modulesBookableCategoriesE = clickedModulesE.snapshotE(moduleBookableCategories).delayE(3);
 var modulesDelE = clickedModulesE.snapshotE(close).delayE(4);

 var modulesAllE = modulesNameE.mergeE(modulesPointsE, modulesBookableCategoriesE, modulesDelE).delayE(5);
 ...
 var modulesCollectedE = modulesAllE.collectE([], function(item, rest) {
  ...
  return rest;
}).mapE(function(a) { return TABLE(TR(TH("Name"), TH("Points"), TH("Bookable Categories")), a); });

 insertDomE(modulesCollectedE, 'ModulesTable');

 totalE = clickedModulesE.snapshotE(modulePoints).collectE(0, function(item, count) { return parseInt(item) + parseInt(count); });
